...
public class PlayerController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        float moveHoritzontal;
        float moveVeritcal;
        Vector2 movement;

        if (Application.platform == RuntimePlatform.Android)
        {
            moveHoritzontal = Input.acceleration.x;
            moveVeritcal = Input.acceleration.y;
            movement = new Vector2(moveHoritzontal, moveVeritcal);
        ...
        else
        {
            ...
            moveVeritcal = Input.GetAxis("Vertical");
            movement = new Vector2(moveHoritzontal, moveVeritcal);
            rb2d.AddForce(movement * speed);
        }
